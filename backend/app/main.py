...
class ChatRequest(BaseModel):
    ...
    mode: str = Field(default="preview", pattern="^(preview|execute|nocode)$")
...
# Path is overridable via DBCHAT_STATIC_DIR (used in the monorepo, where the
# frontend builds to the repo root, not ../frontend/dist).
_static_override = os.getenv("DBCHAT_STATIC_DIR")
_DIST = (
    Path(_static_override)
    if _static_override
